...
namespace IntegrationTests.AuthorsController
{
    public class Put : AuthorsControllerTestBase
    {
        ...
        {
            var authorToPost = new Author() { FullName = "test", TwitterAlias = "test" };
            var jsonContent = new StringContent(JsonConvert.SerializeObject(authorToPost), Encoding.UTF8, "application/json");
            var response = await _client.PutAsync($"/api/{controllerName}/3", jsonContent);
        ...
        {
            var authorToPost = new Author() { FullName = "", TwitterAlias = "test"};
            var jsonContent = new StringContent(JsonConvert.SerializeObject(authorToPost), Encoding.UTF8, "application/json");
            var response = await _client.PutAsync($"/api/{controllerName}/2", jsonContent);
